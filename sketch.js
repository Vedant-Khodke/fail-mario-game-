...
function preload(){
  ...
  castle = loadImage("Mario boi/castle1.png");
}
...
function draw(){
 ...
 // spawnluckybricks(); 
  if(frameCount % 100 == 0){
    ...
    bricks1.depth = mario.depth; 
    //if(mario.isTouching(bricks1)){
      //bricks1.addImage("coin", coins); 
    //}  

    if(mario.y-bricks1.y<mario.height/2+bricks1.height/2
      && bricks1.y-mario.y<mario.height/2+bricks1.height/2){
        bricks1.addImage("coin", coins); 
        score = score+25;
    }
  ...
  if(gameState == "End"){
    luckybricksgroup.destroyEach();
    bricksgroup.destroyEach();
    backgroundsprite.velocityX = 0;
    var castle2 = createSprite(400,235,20,20);
    castle2.addImage("end", castle);
    mario.velocityY = mario.velocityY + 2.1;
    mario.pause();
    mario.collide(inviground);
    if(keyWentDown("A")){
    ...
    castle2.depth = mario.depth;
    mario.depth = castle2.depth+1;
  ...
  if(gameState == "Play"){
    fill("gold");
    textSize(20);
    text("Coins : "+score,width-300,height/5-20);
  }
}
/*
